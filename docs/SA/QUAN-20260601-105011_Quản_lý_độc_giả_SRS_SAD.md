...
|----------------|--------|----------------------------------|------------------|----------------|
| `QUAN-20260601-105011-FR01` | `QUAN-20260601-105011-SRS01` | **Create New Reader:** <br/> 1. Nhận `CreateReaderCommand` (DTO). <br/> 2. Validate DTO (sử dụng FluentValidation), bao gồm kiểm tra tuổi độc giả >= 16 (BR05). <br/> 3. Kiểm tra tính duy nhất của `PhoneNumber` trong DB (BR02). <br/> 4. Nếu `PhoneNumber` đã tồn tại, trả về lỗi. <br/> 5. Tự động sinh `ReaderCode` theo format `DG[YYYYMMDD][SequentialNumber]` (BR01). <br/> 6. Tạo đối tượng `Reader` entity từ DTO. <br/> 7. Lưu `Reader` entity vào DB. <br/> 8. Ghi lại log audit trail (người tạo, thời gian, dữ liệu). <br/> 9. Commit transaction. | - `FullName`: Bắt buộc, chuỗi, tối đa 250 ký tự. <br/> - `DateOfBirth`: Bắt buộc, kiểu ngày, không được lớn hơn ngày hiện tại, độc giả phải từ 16 tuổi trở lên tính đến ngày đăng ký thẻ (BR05). <br/> - `PhoneNumber`: Bắt buộc, chuỗi, định dạng số điện thoại Việt Nam (10 chữ số, bắt đầu bằng 0), duy nhất (BR02). <br/> - `Email`: Không bắt buộc, chuỗi, định dạng email hợp lệ, tối đa 250 ký tự. <br/> - `Address`: Không bắt buộc, chuỗi, tối đa 500 ký tự. <br/> - `RegistrationDate`: Bắt buộc, kiểu ngày, không được lớn hơn ngày hiện tại. <br/> - `ExpiryDate`: Bắt buộc, kiểu ngày, không được nhỏ hơn `RegistrationDate`. <br/> - `Status`: Bắt buộc, enum (mặc định: `Active`). | Trả về `ReaderId` (GUID) và `ReaderCode` của độc giả vừa tạo. |
...
```json
{
  ...
  "errors": [
    ...
    { "field": "dateOfBirth", "message": "Reader must be at least 16 years old." },
